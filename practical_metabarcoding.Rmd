...
Empecemos por el formato de los datos en bruto o raw.
...
```{r}
Fwd <- list.files(path = "Data/fastq", pattern = "*R1.fastq*")
Rev <- list.files(path = "data", pattern = "*R2.fastq*")
...
cutadapt -g FWD=GGWACWGGWTGAACWGTWTAYCCYCC;min_overlap=18 -G REV=TANACYTCNGGRTGNCCRAARAAYCA;min_overlap=18 --discard-untrimmed -e 0.1 -o noprimers/read1_R1.fastq -p noprimers/read1_R2.fastq rawdata/read1.fastq rawdata/read2.fastq
...
```{r}
Fwd <- list.files(path = "Data/nsdna", pattern = "*1.fastq*", full.names = T)
Rev <- list.files(path = "Data/nsdna", pattern = "*2.fastq*", full.names = T)

samples <- tibble (R1 = Fwd, R2 = Rev)|> 
  mutate(sample_name = basename(R1)) |> 
  separate (sample_name, into = c("sample_name"), sep = "_FWD", remove = F) 
```
